...
const API_KEY = "486a30fe2a4040f404391459060015ad";
...
const getLocation = async (searchInput) => {
	const geo_api_url = `https://api.openweathermap.org/geo/1.0/direct?q=${searchInput}&limit=1&appid=${API_KEY}`;
	try {
...
const getForecast = async (locationData) => {
	const latitude = locationData[0].lat;
	const longitude = locationData[0].lon;
	const weather_api_url = `https://api.openweathermap.org/data/2.5/onecall?lat=${latitude}&lon=${longitude}&appid=${API_KEY}`;
	try {
